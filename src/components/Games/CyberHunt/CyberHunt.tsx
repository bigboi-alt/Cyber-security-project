...
export const CyberHunt: React.FC<CyberHuntProps> = ({ student, onPointsEarned }) => {
  ...
  return (
    <div className={`transition-all ${isFullscreen ? 'fixed inset-0 z-50 p-2 bg-[#09090b]' : 'w-full space-y-4 pb-12'}`}>
      ...
      {!isFullscreen && (
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-[#0c0c0f] border-2 border-zinc-800 rounded-xl px-4 py-3 shadow-lg">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-zinc-900 border border-zinc-700 flex items-center justify-center text-white shrink-0 shadow-sm">
              <Globe className="w-4 h-4 text-white" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-mono text-[#9d9e99] uppercase tracking-widest font-bold">
                  [ ACT.01 // FIELD SIMULATION ]
                </span>
                {!student && (
                  <span className="text-[10px] font-mono text-amber-400 bg-amber-950/60 px-2 py-0.2 rounded-full border border-amber-500/50 font-bold uppercase">
                    Guest Mode (No Save)
                  </span>
                )}
              </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-white hover:bg-zinc-200 text-black font-black font-mono text-xs transition-transform active:translate-y-0.5 cursor-pointer shadow-sm uppercase"
            >
      ...
      {/* Main Browser Window Container (Sleek Curvy Modern Browser) */}
      <div className={`rounded-2xl border-2 border-zinc-700/80 bg-[#0c0c0f] shadow-2xl overflow-hidden flex flex-col ${
        isFullscreen ? 'h-full' : 'h-[calc(100vh-140px)] min-h-[700px]'
      }`}>
        
        {/* 1. Window Frame Strip (Mac Dots, Dynamic Tabs, Controls) */}
        <div className="bg-[#18181b] border-b border-zinc-800 px-4 pt-2.5 pb-0 flex items-center justify-between gap-2 overflow-x-auto select-none">
          {/* Window Control Dots (Curvy macOS traffic lights) */}
          <div className="flex items-center gap-2 mr-2 shrink-0">
            <div 
              onClick={() => setIsFullscreen(false)} 
              className="w-3 h-3 rounded-full bg-red-500 hover:bg-red-400 cursor-pointer transition-colors shadow-sm" 
              title="Close Fullscreen"
            />
            <div 
              onClick={() => handleRefresh()} 
              className="w-3 h-3 rounded-full bg-amber-500 hover:bg-amber-400 cursor-pointer transition-colors shadow-sm" 
              title="Reload Tab"
            />
            <div 
              onClick={() => setIsFullscreen(!isFullscreen)} 
              className="w-3 h-3 rounded-full bg-emerald-500 hover:bg-emerald-400 cursor-pointer transition-colors shadow-sm" 
          ...
          <div className="flex items-center gap-1.5 flex-1 overflow-x-auto no-scrollbar">
            {tabs.map((tab) => {
              ...
              return (
                <div
                  ...
                  className={`group flex items-center gap-2 px-3.5 py-1.5 rounded-t-xl text-xs font-medium cursor-pointer transition-colors shrink-0 max-w-[200px] border-t border-x ${
                    isActive
                      ? 'bg-[#09090b] text-white border-zinc-700/80 shadow-sm'
                ...
                >
                  ...
                  {tab.canClose && (
                    <button
                      ...
                      className="p-0.5 rounded-full hover:bg-zinc-700 text-zinc-500 hover:text-white transition-colors cursor-pointer"
                      title="Close tab"
                    >
                      <X className="w-3 h-3" />
                    </button>
            ...
            <button
              ...
              className="p-1.5 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-full transition-colors cursor-pointer shrink-0"
              title="Open New Tab"
            >
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <button
              onClick={() => setShowTodoSidebar(!showTodoSidebar)}
              className={`px-2 py-1 rounded-lg text-xs flex items-center gap-1.5 transition-colors cursor-pointer ${
        ...
        <div className="bg-[#121214] border-b border-zinc-800 px-3 py-2 flex items-center gap-2 text-xs">
          ...
          <div className="flex items-center gap-1 text-zinc-400 shrink-0">
            <button 
              ...
              className={`p-1.5 rounded-full cursor-pointer ${historyIndex > 0 ? 'hover:text-white hover:bg-zinc-800' : 'opacity-40 cursor-not-allowed'}`}
              title="Back"
            >
            ...
            <button 
              ...
              className={`p-1.5 rounded-full cursor-pointer ${historyIndex < navHistory.length - 1 ? 'hover:text-white hover:bg-zinc-800' : 'opacity-40 cursor-not-allowed'}`}
              title="Forward"
            >
            ...
            <button 
              onClick={handleRefresh} 
              className={`p-1.5 hover:text-white rounded-full hover:bg-zinc-800 cursor-pointer transition-transform ${isRefreshing ? 'animate-spin' : ''}`}
              title="Reload Page"
            >
            ...
            <button 
              ...
              className="p-1.5 hover:text-white rounded-full hover:bg-zinc-800 cursor-pointer"
              title="Home Page"
            >
          ...
          {/* Curvy Omnibox URL Bar */}
          <form onSubmit={handleUrlSubmit} className="flex-1 flex items-center gap-2.5 bg-[#18181b] border border-zinc-700/80 focus-within:border-zinc-400 rounded-full px-4 py-1.5 text-zinc-300 font-mono text-xs transition-colors shadow-inner">
            ...
            >
              {urlInput.startsWith('https://') ? (
                <Lock className="w-3.5 h-3.5 text-emerald-400/90 hover:text-emerald-300" />
              ) : (
        ...
        {/* 3. Bookmarks Strip */}
        <div className="bg-[#0e0e10] border-b border-zinc-800/80 px-4 py-1.5 flex items-center gap-2 text-xs text-zinc-400 overflow-x-auto select-none no-scrollbar">
          <button
            onClick={() => navigateCurrentTab('google', 'https://www.google.com', 'Google')}
            className="hover:text-white px-2.5 py-1 rounded-full hover:bg-zinc-800/80 flex items-center gap-1.5 cursor-pointer transition-colors shrink-0"
          >
          ...
          <button
            ...
            className="hover:text-white px-2.5 py-1 rounded-full hover:bg-zinc-800/80 flex items-center gap-1.5 cursor-pointer transition-colors shrink-0 ml-auto"
          >
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 bg-[#09090b] text-zinc-200 overflow-y-auto p-4 sm:p-6">
              ...
              <div className="max-w-2xl mx-auto space-y-6 pt-4 animate-in fade-in duration-200">
                <div className="text-center space-y-4">
                  ...
                  <form
                    ...
                    className="flex items-center gap-3 max-w-lg mx-auto bg-[#18181b] border-2 border-zinc-700 hover:border-zinc-500 rounded-full px-5 py-3 shadow-lg transition-colors"
                  >
                ...
                {hasSearched && (
                  <div className="space-y-4 pt-4 border-t border-zinc-800">
                    ...
                    <div className="p-4 rounded-xl bg-[#121214] border border-zinc-800 hover:border-zinc-700 transition-colors space-y-1.5 shadow-sm">
            ...
            {activeTab.type === 'amazon_fake' && (
              <div className="max-w-sm mx-auto p-6 bg-[#121214] border-2 border-zinc-800 rounded-2xl space-y-4 text-center animate-in fade-in duration-200 shadow-xl">
                ...
                <form onSubmit={handleFakeAmazonSubmit} className="space-y-3 text-left">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full mt-1 bg-[#18181b] border border-zinc-700 focus:border-amber-400 rounded-lg p-2.5 text-xs text-white font-mono outline-none"
                    />
                  </div>
                  <div>
                  ...
                  <button
                    type="submit"
                    className="w-full py-2.5 rounded-xl bg-amber-400 hover:bg-amber-300 text-black font-bold text-xs cursor-pointer transition-colors shadow-md font-mono uppercase"
                  >
            ...
            {activeTab.type === 'amazon_legit' && (
              <div className="max-w-3xl mx-auto space-y-4 animate-in fade-in duration-200">
                <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
                  ...
                  <div className="flex items-center gap-1.5 text-xs font-mono text-green-400 bg-green-950/20 border border-green-800/50 px-2.5 py-0.5 rounded-full">
                ...
                <div className="p-5 bg-[#121214] border border-zinc-800 rounded-2xl flex flex-col md:flex-row items-center gap-5 shadow-lg">
                  <div className="w-32 h-36 rounded-xl bg-zinc-900 border border-zinc-700 flex flex-col items-center justify-center text-white shrink-0 p-3 text-center">
                  ...
                  <div className="flex-1 space-y-2 text-left">
                    <div className="flex items-center gap-2">
                      <span className="px-2.5 py-0.5 rounded-full bg-zinc-800 text-zinc-300 text-[10px] font-mono border border-zinc-700">
                        Official Curriculum
                      </span>
                  ...
                  <div className="shrink-0 w-full md:w-auto">
                    <button
                      ...
                      className="w-full md:w-auto px-5 py-3 rounded-xl bg-amber-400 hover:bg-amber-300 text-black font-bold text-xs cursor-pointer transition-colors shadow-md flex items-center justify-center gap-2 font-mono uppercase"
                    >
              ...
              <div className="max-w-4xl mx-auto space-y-4 animate-in fade-in duration-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="p-3 rounded-xl bg-[#16161a] border border-zinc-700 space-y-1 cursor-pointer shadow-sm">
                  ...
                  <div className="md:col-span-2 bg-[#121214] border border-zinc-800 rounded-2xl p-5 space-y-4 shadow-lg">
                    ...
                    <div className="p-4 rounded-xl bg-[#18181b] border border-zinc-700/80 text-center space-y-2.5 shadow-sm">
                      ...
                      <button
                        ...
                        className="px-4 py-2 rounded-xl bg-white hover:bg-zinc-200 text-black font-bold text-xs cursor-pointer transition-colors inline-flex items-center gap-1.5 font-mono uppercase shadow-sm"
                      >
                    ...
                    {task2Done && (
                      <div className="p-3 rounded-xl bg-green-950/20 border border-green-800/60 text-xs text-green-300 flex items-center gap-2 font-mono">
            ...
            {activeTab.type === 'party_invite' && (
              <div className="max-w-md mx-auto p-6 bg-[#121214] border-2 border-zinc-800 rounded-2xl space-y-4 text-center animate-in fade-in duration-200 shadow-xl">
                <PartyPopper className="w-10 h-10 text-amber-400 mx-auto" />
                <h3 className="text-base font-bold text-white font-mono">
                  Confirm Birthday Party Attendance
                </h3>

                <div className="bg-[#18181b] border border-zinc-700/80 rounded-xl p-3.5 text-left space-y-2.5 text-xs">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full mt-1 bg-zinc-900 border border-zinc-700/80 rounded-lg p-2 text-xs text-zinc-300 font-mono outline-none"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      className="w-full mt-1 bg-zinc-900 border border-zinc-700/80 rounded-lg p-2 text-xs text-white font-mono outline-none"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 pt-2">
                  <button
                    ...
                    className="py-2.5 px-3 rounded-xl border border-zinc-700 bg-zinc-900 hover:bg-zinc-800 text-zinc-200 font-bold text-xs transition-colors cursor-pointer font-mono uppercase shadow-sm"
                  >
                    Submit RSVP Details
                  </button>
            ...
            {activeTab.type === 'download_portal' && (
              <div className="max-w-3xl mx-auto space-y-5 animate-in fade-in duration-200">
                <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
                  ...
                  <span className="text-xs font-mono text-green-400 bg-green-950/20 px-2.5 py-0.5 rounded-full border border-green-800/40">
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 rounded-2xl border-2 border-zinc-800 bg-[#121215] text-center space-y-2 shadow-sm">
                    ...
                    <button
                      ...
                      className="py-3 px-6 rounded-xl bg-green-500 hover:bg-green-400 text-black font-black text-sm cursor-pointer transition-colors shadow-lg animate-pulse font-mono uppercase"
                    >
                  ...
                  {/* Real Document Link */}
                  <div className="p-4 rounded-2xl bg-[#121214] border border-zinc-800 hover:border-zinc-700 transition-colors flex items-center justify-between shadow-sm">
                    <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      className="px-4 py-2 rounded-xl bg-zinc-800 hover:bg-zinc-700 border border-zinc-600 text-white font-semibold text-xs cursor-pointer transition-colors flex items-center gap-1.5 font-mono shadow-sm"
                    >
            ...
            {activeTab.type === 'history' && (
              <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-200">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="bg-[#18181b] border border-zinc-700 rounded-full pl-9 pr-4 py-1.5 text-xs text-white placeholder:text-zinc-500 focus:outline-none focus:border-zinc-400 w-56 font-mono"
                      />
                    </div>

                    <button
                      onClick={clearAllHistory}
                      className="px-3.5 py-1.5 rounded-full bg-zinc-800 hover:bg-zinc-700 text-zinc-300 hover:text-white text-xs font-medium cursor-pointer transition-colors border border-zinc-700 font-mono"
                    >
                      Clear Data
                    </button>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="p-4 rounded-xl bg-[#121214] border border-zinc-800 space-y-1 shadow-sm">
                ...
                <div className="space-y-2">
                  {filteredHistory.length === 0 ? (
                    <div className="p-8 text-center text-zinc-500 text-xs bg-[#121214] rounded-xl border border-zinc-800 font-mono">
                    ...
                    filteredHistory.map((item) => (
                      <div
                        key={item.id}
                        className="p-3.5 rounded-xl bg-[#121214] border border-zinc-800/80 hover:border-zinc-700 transition-colors flex items-center justify-between gap-3 text-xs shadow-sm"
                      >
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2">
                              ...
                              {item.category === 'phishing' && (
                                <span className="px-2 py-0.5 rounded-full bg-red-950/60 border border-red-500/50 text-red-300 text-[10px] font-mono shrink-0">
                                  Typosquat Threat
                                </span>
                              )}
                              {item.category === 'threat' && (
                                <span className="px-2 py-0.5 rounded-full bg-amber-950/60 border border-amber-500/50 text-amber-300 text-[10px] font-mono shrink-0">
                                  Credential Harvest
                                </span>
                              )}
                              {item.category === 'safe' && (
                                <span className="px-2 py-0.5 rounded-full bg-green-950/40 border border-green-600/40 text-green-300 text-[10px] font-mono shrink-0">
                                  Verified Safe
                                </span>
                              )}
                            </div>
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          ...
                          <button
                            ...
                            className="p-1.5 text-zinc-500 hover:text-red-400 rounded-full hover:bg-zinc-800 cursor-pointer"
                            title="Delete entry"
                          >
              ...
              <div className="max-w-2xl mx-auto space-y-6 pt-8 text-center animate-in fade-in duration-200">
                ...
                <div className="pt-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 max-w-md mx-auto">
                    <button
                      onClick={() => navigateCurrentTab('google', 'https://www.google.com', 'Google')}
                      className="p-4 bg-[#121214] border border-zinc-800 hover:border-zinc-600 rounded-2xl space-y-1.5 transition-colors cursor-pointer text-center shadow-sm"
                    >
          ...
          {showTodoSidebar && (
            <div className="w-80 border-l-2 border-zinc-800 bg-[#101013] flex flex-col shrink-0 select-none overflow-y-auto">
              ...
              <div className="p-3 space-y-3 flex-1 overflow-y-auto">
                ...
                <div className={`p-3.5 rounded-xl border transition-all ${
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2">
                      {task1Done ? (
                        ...
                        <div className="w-5 h-5 rounded-full border border-zinc-500 flex items-center justify-center text-[10px] font-mono text-zinc-300">
                          1
                        </div>
                      )}
                  ...
                  {!task1Done && (
                    <button
                      ...
                      className="mt-2.5 w-full py-2 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-white font-medium text-xs flex items-center justify-center gap-1 cursor-pointer transition-colors font-mono"
                    >
                ...
                {completedCount === 3 && (
                  <div className="p-4 rounded-xl bg-green-950/30 border border-green-500/60 text-center space-y-2 shadow-sm">
      ...
      {showCertModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="max-w-sm w-full bg-[#141417] border border-zinc-700 rounded-2xl p-5 text-left space-y-4 shadow-2xl">
            ...
            <button
              onClick={() => setShowCertModal(false)}
              className="w-full py-2.5 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-white font-medium text-xs cursor-pointer transition-colors font-mono uppercase shadow-sm"
            >
              Close Inspector
            </button>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="max-w-md w-full bg-[#121214] border border-zinc-800 rounded-2xl p-6 text-center space-y-4 shadow-2xl">
            ...
            <button
              ...
              className="w-full py-2.5 rounded-xl bg-white hover:bg-zinc-200 text-black font-semibold text-xs cursor-pointer transition-colors font-mono uppercase shadow-sm"
            >
              Continue Browsing
            </button>
          </div>

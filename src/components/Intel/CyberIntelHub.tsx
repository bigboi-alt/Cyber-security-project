...
export const CyberIntelHub: React.FC<CyberIntelHubProps> = ({
  ...
  return (
    <div className="space-y-6 pb-12 animate-in fade-in duration-200">
      {/* Header Banner */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#0c0c0f] border-2 border-zinc-800 rounded-none p-5 comic-shadow blocky-card-dark">
        <div>
          <div className="text-[10px] font-mono uppercase tracking-widest text-[#9d9e99] font-bold">
            [ OPERATIVE INTEL ARCHIVE // DEFENSE MANUAL ]
          </div>
        ...
        <div className="flex items-center gap-3 text-xs font-mono">
          <span className="bg-zinc-900 border border-zinc-700 px-3 py-1.5 rounded-none text-zinc-300 font-bold">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="text-white bg-[#141418] hover:bg-zinc-800 border-2 border-zinc-700 px-3 py-1.5 rounded-none flex items-center gap-1.5 font-bold transition-colors comic-shadow-sm blocky-btn"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
            ...
            {TOPICS.map((topic) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-3.5 py-2.5 rounded-none text-xs font-mono font-bold transition-all cursor-pointer flex items-center justify-between border-2 ${
                    isSelected
        ...
        <div className="md:col-span-3 space-y-4">
          ...
          <div className="space-y-3">
            {filteredItems.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="rounded-none bg-[#0c0c0f] border-2 border-zinc-800 p-4 transition-all hover:border-zinc-600 space-y-2 comic-shadow blocky-card-dark"
                >
                  <div className="flex items-start gap-3.5">
                    <div className="w-9 h-9 rounded-none bg-zinc-900 border-2 border-zinc-700 flex items-center justify-center text-white shrink-0 mt-0.5 comic-shadow-sm">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-mono px-2 py-0.2 rounded-none bg-zinc-900 text-zinc-400 border border-zinc-800 uppercase font-semibold">
                          {item.topic}
                        </span>
                      </div>
                      ...
                      <div className="mt-2.5 text-[11px] text-zinc-300 bg-[#141418] border-l-4 border-l-white border-y border-r border-zinc-800/80 p-2.5 rounded-none font-mono">
          ...
          <div className="rounded-none bg-[#0c0c0f] border-2 border-zinc-800 p-5 mt-6 comic-shadow space-y-3 blocky-card-dark">
            <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
              <div className="flex items-center gap-2">
                <Bookmark className="w-4 h-4 text-white" />
                <h4 className="text-xs font-black text-white font-mono uppercase tracking-widest">
                  OPERATIVE COMPLIANCE CHECKLIST // AUDIT
                </h4>
              </div>
              <span className="text-[10px] font-mono text-emerald-400 bg-emerald-950/60 px-2 py-0.5 rounded-none border border-emerald-700/60 font-bold uppercase">
              ...
              {CHECKLIST.map((item, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`p-3 rounded-none border-2 text-xs flex items-center gap-3 cursor-pointer transition-all ${
                      isChecked
